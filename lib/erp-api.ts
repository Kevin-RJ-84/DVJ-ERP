...
const ERP_COMMAND_TYPE_STOCK = process.env.ERP_COMMAND_TYPE_STOCK ?? "GETDATA";
const ERP_COMMAND_TYPE_SALES = process.env.ERP_COMMAND_TYPE_SALES ?? "GETDATA";
 ...
 */
export async function getErpToken(remoteAddress?: string): Promise<string> {
  ...
  const resolvedRemoteAddress =
    remoteAddress ?? process.env.ERP_REMOTE_ADDRESS ?? "";
 ...
 */
export async function fetchErpStock(remoteAddress?: string): Promise<ErpStockRecord[]> {
  const token = await getErpToken(remoteAddress);
  ...
  const response = await fetch(`${ERP_BASE_URL}/api/JewelryReport/getJewelryStock`, {
    ...
    body: JSON.stringify({
      command_type: ERP_COMMAND_TYPE_STOCK,
      user_id: ERP_USER_ID,
    }),
  });
 ...
 */
export async function fetchErpSales(remoteAddress?: string): Promise<ErpSaleRecord[]> {
  const token = await getErpToken(remoteAddress);
  ...
  const response = await fetch(`${ERP_BASE_URL}/api/JewelryReport/getJewelrySale`, {
    ...
    body: JSON.stringify({
      command_type: ERP_COMMAND_TYPE_SALES,
      user_id: ERP_USER_ID,
    }),
  });
